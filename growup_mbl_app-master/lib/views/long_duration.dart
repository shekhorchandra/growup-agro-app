...
import '../models/long_project_model.dart';
...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return const MaterialApp(
      ...
      home: LongProjectsPage(),
    );
  }
}

class LongProjectsPage extends StatefulWidget {
  final bool hideAppBar;
  const LongProjectsPage({super.key, this.hideAppBar = false});

  @override
  State<LongProjectsPage> createState() => _LongProjectsPageState();
}

class _LongProjectsPageState extends State<LongProjectsPage> {
  Set<int> _loadingProjectIds = {};
  late Future<List<LongProjectModel>> futureLongProjects;
  final ScrollController _scrollController = ScrollController();
  final TextEditingController _searchController = TextEditingController();
  List<LongProjectModel> _allProjects = [];
  List<LongProjectModel> _filteredProjects = [];
  bool _showBackToTopButton = false;

  @override
  void initState() {
    super.initState();
    futureLongProjects = fetchLongTermProjects();
  ...
  Future<List<LongProjectModel>> fetchLongTermProjects() async {
    ...
    if (response.statusCode == 200) {
      ...
      if (longProjects == null) {
        throw Exception('Long Term projects not found.');
      }

      _allProjects = longProjects
          .map<LongProjectModel>((project) => LongProjectModel.fromJson(project))
          .toList();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: FutureBuilder<List<LongProjectModel>>(
        future: futureLongProjects,
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            ...
            return const Center(child: Text('No Long term projects found.'));
          }
